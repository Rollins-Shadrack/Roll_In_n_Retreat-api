...
const partnerOnboardTemple = require('../../emailTemplates/partnerOnboard');
...
const registerUser = asyncHandler(async (req, res, next) => {
    try {
        await prisma.$transaction(async (PrismaClient) => {
            ...
            if (checkEmail) return res.status(400).json(checkEmail.message);
...
const registerPartner = asyncHandler(async (req, res, next) => {
    try {
        await prisma.$transaction(async (PrismaClient) => {
            const { ipAddress, useragent } = agentDetails;
            const checkEmail = await emailExist(req.body.email, PrismaClient)
            ...
            const createTempReg = await createTempRegistration(req.body, 'partner', PrismaClient);
            if (!createTempReg) return res.status(400).json('could not create resource')
            ...
            const fullName = `${req.body.firstName} ${req.body.lastName}`;
            if (createTempReg) await sendEmail(req.body.email, "Welcome to Roll In & Retreat! Complete Your Account Setup Now!", partnerOnboardTemple(fullName, createTempReg.link));
            // console.log(createTempReg);
            res.status(201).json(createTempReg);
        })
    } catch (error) {
      console.error(error);
